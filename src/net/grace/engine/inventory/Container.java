package net.grace.engine.inventory;

import net.grace.engine.save.InventorySaveData;
...
public class Container {
	...
	public net.grace.engine.save.InventorySaveData saveContainer() {
		net.grace.engine.save.InventorySaveData.Slot[][] slots = new net.grace.engine.save.InventorySaveData.Slot[rows][columns];
		
		for (int y = 0; y < columns; y++) {
			for (int x = 0; x < rows; x++) {
				Slot s = this.slots[x][y];
				if(s != null) {
					slots[x][y] = new net.grace.engine.save.InventorySaveData.Slot(s.getItem().getID(), s.getCurrentAmount());
				}
			}
		}
	...
	public static Container loadSavedContainer(InventorySaveData data) {
		...
		for (int y = 0; y < data.columns; y++) {
			for (int x = 0; x < data.rows; x++) {
				net.grace.engine.save.InventorySaveData.Slot slot = data.getSlots()[x][y];
				if(slot != null) {
